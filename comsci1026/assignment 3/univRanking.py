...
#description: write average university score on the outpput.txt file
def avgUniScore(selectedCountry, topUniFile):

    avg = findAvg(selectedCountry, topUniFile)
...
def relativeUniScore(selectedCountry, topUniFile, capitalsFile):
    ...
    # write on the output.txt file
    writeOnOutputFile(output)

##### PART 8 & 9

# Name: findCapital
# param: selected country, capital.csv file
# Description: finds the capital for the selected country
def findCapital(selectedCountry, capitalsFile):
    
    ##convert capital file to list
    infos = csvToList(capitalsFile)

    ##finding the index of country and capital
    countryIndex = findIndex("Country Name", infos)
    capitalIndex = findIndex("Capital", infos)

    ##loop through the infos list to find the capital
    for info in infos:
        #if the country is same with selected country, initialize capital
        if info[countryIndex].upper() == selectedCountry.upper():
            capital = info[capitalIndex]
            break
...
# Name: capitalCity
# param: selected country, capital.csv file
# description: finds the capital city of the selected country than writes on the output.txt file
def capitalCity(selectedCountry, capitalsFile):

    #find capital 
    capital = findCapital(selectedCountry, capitalsFile)

    #create output
    output = "The capital is => " + capital.upper()
...
### PART 9
# Name: capitalNameUni
# param: selected country, TopUni.csv, capital.csv
# Description: finds university that includes capital of selected country in its name

def capitalNameUni(selectedCountry, topUniFile, capitalsFile):
    
    #convert csv file to list
    infos = csvToList(topUniFile)

    #find index for the univerrsity name
    uniNameIndex = findIndex("Institution name", infos)

    #find capital of the selected country
    capital = findCapital(selectedCountry, capitalsFile)

    #empty string for list of the university
    uniList = ""
    #varaible for uni count
    uniCount = 0

    #go through the list of TopUni.csv file and find every university with the capital in its name
    for info in infos:
        if capital in info[uniNameIndex]:
            uniCount += 1
            uniList += "\n    #%d %s" % (uniCount, info[uniNameIndex].upper())
        
    #create output
    output = "The universities that contain the capital name =>" + uniList

    #write the file onto the output.txt file
    writeOnOutputFile(output)


#### FINAL TEST FUNCTION
# Name: getInformation
# param: selectedCountry, TopUni.csv, capitals.csv
#description: collection of above functions
def getInformation(selectedCountry, topUniFile, capitalsFile):
    ...
    #part 1
    uniCounts(topUniFile)
    #part 2
    availableCountries(topUniFile)
    #part 3
    availableContinents(capitalsFile)
    #part 4
    highestRankUni(selectedCountry, topUniFile)
    #part 5
    highestNationalRankUni(selectedCountry, topUniFile)
    #part 6
    avgUniScore(selectedCountry, topUniFile)
    #part 7
    relativeUniScore(selectedCountry, topUniFile, capitalsFile)
    #part 8
    capitalCity(selectedCountry, capitalsFile)
    #part 9
    capitalNameUni(selectedCountry, topUniFile, capitalsFile)
